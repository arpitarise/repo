...
const SETTINGS_KEY = 'arise_sys_settings_v3';
...
const DEFAULT_SETTINGS = {
  E: {
    ...
    lateSlabs: [
      ...
      { id: 3, start: 71, end: 999, value: -1.5 }
    ],
    earlyGrace: 0,
    earlySlabs: [
      { id: 1, start: 10, end: 45, value: -0.5 },
      { id: 2, start: 46, end: 74, value: -1.0 },
      { id: 3, start: 75, end: 95, value: -1.5 },
      { id: 4, start: 96, end: 999, value: -2.0 }
    ],
    otGrace: 44,
    otSlabs: [
      { id: 1, start: 45, end: 74, value: 0.5 },
      { id: 2, start: 75, end: 104, value: 1.0 },
      { id: 3, start: 105, end: 134, value: 1.5 },
      { id: 4, start: 135, end: 999, value: 2.0 }
    ],
    earlyInSlabs: [
      { id: 1, start: 40, end: 70, value: 0.5 },
      { id: 2, start: 71, end: 120, value: 1.0 }
    ],
...
const selectLocalDirectory = async () => {
  ...
  } catch (err) { notify("Directory selection cancelled or failed.", "error"); }
};
const triggerLocalUnlock = async () => {
  ...
  } catch (err) { notify("Folder unlock request failed.", "error"); }
};
...
const handleExportExcel = () => {
  ...
  try {
    ...
    db.forEach((emp, index) => {
      ...
      const empType = String(emp.type || "").trim().toUpperCase() || "E";
      const rule = sysSettings[empType] || sysSettings['E'] || sysSettings['S'];
      const getsExtraWoSunday = rule?.extraWoOnSunday ?? true;
      for (let i = 1; i <= activeConf.days; i++) {
        const isWk = activeConf.weekends.includes(i);
      ...
      row.push(
        ...
        cell(`COUNTIFS(F${R}:${endAttColName}${R},"P",F$4:${endAttColName}$4,"<>OT")+COUNTIFS(F${R}:${endAttColName}${R},"P~?",F$4:${endAttColName}$4,"<>OT")+COUNTIFS(F${R}:${endAttColName}${R},"H",F$4:${endAttColName}$4,"<>OT")*0.5+COUNTIFS(F${R}:${endAttColName}${R},"0.5",F$4:${endAttColName}$4,"<>OT")*0.5+COUNTIFS(F${R}:${endAttColName}${R},"L",F$4:${endAttColName}$4,"<>OT")`, null, true, "center", true),
        cell(`COUNTIFS(F${R}:${endAttColName}${R},"W/O",F$4:${endAttColName}$4,"<>OT") + IF(1=1, ${extraWoFormula}, 0)`, null, false, "center", true),
        cell(`COUNTIFS(F${R}:${endAttColName}${R},"T",F$4:${endAttColName}$4,"<>OT")`, null, false, "center", true),
        cell(`${colP}${R}+${colWO}${R}+${colT}${R}`, null, true, "center", true),
        cell(`COUNTIFS(F${R}:${endAttColName}${R},"A",F$4:${endAttColName}$4,"<>OT")`, null, true, "center", true),
  ...
  } catch(err) { notify("Failed to export Excel", "error"); }
};
